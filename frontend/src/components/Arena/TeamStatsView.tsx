...
export default function TeamStatsView() {
    ...
    const { data: statsData, loading: statsLoading, error: statsError } = useTeamGameStats(selectedAbbr, season)
            ...
            <aside style={{
                ...
                <div style={{ padding: '16px 14px 10px', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <select
                        ...
                        value={season}
                        onChange={e => { setSeason(e.target.value); setSelectedAbbr(null) }}
                        style={{ width: '100%' }}
                    >
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                    ...
                    {!teamsLoading && teams.map(t => (
                        <TeamRow
                            ...
                            onClick={() => setSelectedAbbr(t.abbreviation)}
                        />
                    ))}
                </div>
            </aside>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '28px 28px' }}>
                ...
                {selectedAbbr && statsData && !statsLoading && (
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
                        ...
                        {/* Record cards */}
                        <p className="section-label" style={{ color: ACCENT, marginBottom: 12 }}>Season Record</p>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 10, marginBottom: 28 }}>
                            <RecordCard label="Record" value={`${statsData.record.wins}-${statsData.record.losses}`} />
                            <RecordCard label="Games" value={statsData.record.games} />
                            <RecordCard label="Win %" value={statsData.record.games > 0 ? ((statsData.record.wins / statsData.record.games) * 100).toFixed(1) + '%' : '—'} />
                        </div>
                        ...
                        <div style={{
                            ...
                            <div style={{ overflowX: 'auto' }}>
                                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
                                    ...
                                    <tbody>
                                        {(statsData.games as TeamGameLogEntry[]).map((g, i) => (
                        ...
                        {statsData.games.length === 0 && (
                            <div style={{ padding: '20px', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 'var(--r-md)', marginTop: 12 }}>
                                <p style={{ fontSize: '0.85rem', color: 'var(--text-2)' }}>No games found for this team in {season}.</p>
                            </div>
                        )}
                    </motion.div>
                )}
