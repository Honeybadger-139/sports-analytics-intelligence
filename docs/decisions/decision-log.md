...
| 15 | **Incremental DB Sync & Complete Player Lists** | Truncate and load daily, looping through 30 teams for active rosters | Pulling `MAX(game_date)` from the DB to filter the game logs API response prevents redundant upserts, speeding up daily ingestion immensely. Changing roster logic from `CommonTeamRoster` (active players only) to `CommonAllPlayers` captures historical/traded/waived players efficiently in a single API call. | `CommonAllPlayers` doesn't provide precise positions, requiring a sacrifice of minor player metadata in Phase 1 setup. | "Instead of a naive 'truncate and reload' or repeating heavy upserts daily, I implemented watermarking: selecting the max date from the database and fetching only games on or after that date. This optimized our API rate limits and lowered database IO." |
| 16 | **Database** | `DECIMAL(8,2)` for player season totals | `DECIMAL(5,2)` (used for per-game stats) | Season total points/mins easily exceed 999.99; prevents numeric overflow | Slight storage overhead | "Always model constraints around the logical lifetime maximum, not the typical average." |
