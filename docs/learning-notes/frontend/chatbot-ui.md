...
A full-page AI assistant embedded in the `frontend` React app. Users type natural-language questions about NBA analytics (stats, news, predictions) and receive answers generated by a hybrid RAG+SQL backend engine.

**Architecture at a glance**:
```
User types question
        ↓
useChatbot hook → POST /api/v1/chat
        ↓
Backend: IntentRouter → RAG path or DB path
        ↓
...
### 1. `useChatbot.ts` — state and network logic
...
```ts
// AbortController pattern — prevents "setState on unmounted component" warnings
...
**Future migration path**: If chat history needs to persist across navigation, promote the hook state to Zustand — the interface stays the same, only the storage mechanism changes.

### 2. `ChatMessage.tsx` — pure rendering
...
- Pure component: no hooks, no side effects, just `props → JSX`. Easy to snapshot-test.

### 3. `ChatbotPanel.tsx` (inside `Chatbot.tsx`) — composition and layout
...
- Renders the input form with `onSubmit` → `useChatbot.sendMessage`
- Shows typing indicator when `loading = true`

**Why not inline in `Chatbot.tsx`?**
The panel can be mounted as a sidebar or modal in future iterations. Extracting it means adding a floating chatbot widget later requires zero refactoring — just mount `<ChatbotPanel />` wherever needed.
...
The chatbot uses `--accent-chat: #F59E0B` (amber) — the same token reserved in the design system before the feature was built.
...
**Gotcha**: Amber is also used for `--warning` signals. Avoid using amber for error/warning states on the chatbot page — use `--error: #EF4444` (red) for failed requests instead.
...
> "The AbortController in `useChatbot` prevents React's 'setState on unmounted component' warning. When the user navigates away mid-request, the cleanup function cancels the fetch — this is production-grade hook design."
